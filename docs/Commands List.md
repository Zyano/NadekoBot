...
`.getchannels_cmd` | getchannels_desc **Requires ManageServer server permission.** | getchannels_usage
`.setinfochannel_cmd` | setinfochannel_desc **Requires ManageServer server permission.** | setinfochannel_usage
`.setmusicchannel` `.smch` | Sets the current channel as the default music output channel. This will output playing, finished, paused and removed songs to that channel instead of the channel where the first song was queued in. **Requires ManageServer server permission.** | `.smch`
